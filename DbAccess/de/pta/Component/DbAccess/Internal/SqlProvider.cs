...
namespace de.pta.Component.DbAccess.Internal
{
	...
	internal class SqlProvider : DataProvider
	{
		...
		/// </summary>
		private SqlConnection      mReaderConnection;
		...
		{
			mConnection       = new SqlConnection(mConfig.ConnectString);
			mReaderConnection = new SqlConnection(mConfig.ConnectString);
		}	
		...
		public override IDbCommand CreateCommand(string pId)
		{
			...
			if(null == cmd)
			{
				mLog.Fatal("Could not instantiate SqlCommand object for <" + pId + ">");
		...
		public override IDbDataAdapter CreateDataAdapter(string pId)
		{
			...
			if(null == adapter)
			{
				mLog.Fatal("Could not instantiate SqlDataAdapter object for <" + pId + ">");
			...
			if(!config.ReadOnly) 
			{
				...
				if(null == builder)
				{
					mLog.Fatal("Could not instantiate SqlCommandBuilder object for <" + pId + ">");
		...
		{
			SqlCommand cmd = (SqlCommand)pCommand;
		...
		{
			SqlConnection con = (SqlConnection)pConnection;
		...
		#endregion // End of DataProvider members
		...
		/// <returns></returns>
		private SqlDbType GetSqlType(string pTypeName)
		{
		...
		private void AddParameters(SqlCommand pCommand, BaseConfiguration pConfig)
		{
			...
			while(enumerator.MoveNext())
			{
				...
				dataType = GetSqlType(dbParam.DbType);
				if(dbParam.Precision > 0)
				{
					prm = new SqlParameter(dbParam.Name, dataType);
					prm.Precision = dbParam.Precision;
					prm.Scale = dbParam.Scale;
				}
				else if(dbParam.Length > 0) 
				{
					prm = new SqlParameter(dbParam.Name, dataType, dbParam.Length);
				}
				else
				{
					prm = new SqlParameter(dbParam.Name, dataType);
				...
				if(null == prm)
				{
					mLog.Fatal("Could not instantiate SqlParameter object for <" + pConfig.Id + ">");
